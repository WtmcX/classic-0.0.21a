...
public class Textures {
	...
	public IntBuffer idBuffer = GLAllocation.createDirectIntBuffer(1);
	...
	public final int getTextureId(String var1) {
		Integer integer = (Integer) idMap.get(var1);
		if (integer != null) {
		...
		try {
			...
			int i = idBuffer.get(0);
			addTexture(readTextureImage(PNGLoader.loadResourceBytes(var1)), i);
			idMap.put(var1, Integer.valueOf(i));
			return i;
	...
	private MinecraftImageData readTextureImage(byte[] inputstream) throws IOException {
		return PNGLoader.loadPNG(inputstream);
	}
